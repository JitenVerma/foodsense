import { z } from "zod";
import { DEFAULT_MAX_UPLOAD_SIZE_BYTES, SUPPORTED_IMAGE_MIME_TYPES, } from "../constants/file.js";
...
export const SaveMealRequestSchema = z.object({
    ...
    isLibraryTemplate: z.boolean().optional(),
    sourceMealId: z.string().trim().min(1).nullable().optional(),
    ingredients: z.array(IngredientSchema).min(1),
    assumptions: z.array(z.string().trim().min(1)),
    warnings: z.array(z.string().trim().min(1)),
});
...
export const SavedMealSchema = z.object({
    ...
    isLibraryTemplate: z.boolean(),
    sourceMealId: z.string().trim().min(1).nullable(),
